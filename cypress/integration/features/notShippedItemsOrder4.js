 ...
 * showing for cancelled items in an order scenario 4
 * see ticket: https://chewyinc.atlassian.net/browse/CSRBT-2705
 */
describe.skip('Test Not Shipped Items Order', () => {
  before(() => cy.login());

  const getCustomerActivityFeed = (customerId='267345285') =>
    cy.intercept('GET', `/api/v3/activities/?customerId=${customerId}**`, {
        fixture: `/activities/notShippedItemsOrderActivities4`,
      }).as('activities');

  const getOrder = () =>
    cy
      .intercept('GET', `/api/v3/order-activities/**`, {
        fixture: `/activities/notShippedItemsOrder4`,
      }).as('orders');

  const visitAndFetch = () => {
    getCustomerActivityFeed();
    getOrder();
    cy.visitActivityFeed({ customerId:'267345285'});
    cy.wait(activities);
    cy.wait(orders);
  };
